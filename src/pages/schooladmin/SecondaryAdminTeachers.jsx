...
import { Search, Plus, Eye, Edit2, Trash2, Upload, AlertTriangle, CheckCircle, MoveRight, Clock, CheckCircle2, Check, Camera } from 'lucide-react'

const initialTeachers = [
    { id: 'TCH-S-001', name: 'Mr. Kenneth Okello', photo: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?w=150&auto=format&fit=crop&q=80', subject: 'Mathematics', class: 'S3, S4', phone: '+256 779 123456', email: 'k.okello@kps.ug', status: 'active', attendance: 96, leaveReason: '', leaveStart: '', leaveEnd: '', leaveNotes: '' },
    { id: 'TCH-S-002', name: 'Ms. Agnes Nassali', photo: 'https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?w=150&auto=format&fit=crop&q=80', subject: 'English', class: 'S1, S2', phone: '+256 752 234567', email: 'a.nassali@kps.ug', status: 'active', attendance: 98, leaveReason: '', leaveStart: '', leaveEnd: '', leaveNotes: '' },
    { id: 'TCH-S-003', name: 'Mr. Ivan Byaruhanga', photo: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&auto=format&fit=crop&q=80', subject: 'Physics', class: 'S5, S6', phone: '+256 701 345678', email: 'i.byaruhanga@kps.ug', status: 'on-leave', attendance: 88, leaveReason: 'Leave', leaveStart: '2026-02-15', leaveEnd: '2026-03-01', leaveNotes: 'Annual leave' },
    { id: 'TCH-S-004', name: 'Ms. Patricia Acen', photo: 'https://images.unsplash.com/photo-1580489944761-15a19d654956?w=150&auto=format&fit=crop&q=80', subject: 'History', class: 'S3, S4', phone: '+256 780 456789', email: 'p.acen@kps.ug', status: 'active', attendance: 100, leaveReason: '', leaveStart: '', leaveEnd: '', leaveNotes: '' },
    { id: 'TCH-S-005', name: 'Mr. Samuel Waiswa', photo: 'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?w=150&auto=format&fit=crop&q=80', subject: 'Chemistry', class: 'S5, S6', phone: '+256 755 567890', email: 's.waiswa@kps.ug', status: 'active', attendance: 94, leaveReason: '', leaveStart: '', leaveEnd: '', leaveNotes: '' },
]

const absenceReports = [
    { id: 1, date: 'Feb 21, 2026', time: '8:30 AM', teacher: 'Mr. Kenneth Okello', subject: 'Mathematics', reporter: 'Ivan Namukasa (S4A)', details: 'Teacher did not show up for the entire lesson.', status: 'pending' },
    { id: 2, date: 'Feb 19, 2026', time: '11:00 AM', teacher: 'Mr. Ivan Byaruhanga', subject: 'Physics', reporter: 'Sarah Kemigisha (S5B)', details: 'Arrived 40 minutes late.', status: 'reviewed' },
]

const allSecondaryClasses = ['S1A', 'S1B', 'S2', 'S3', 'S4A', 'S4B', 'S5', 'S6']
const allSecondarySubjects = ['Mathematics', 'English', 'Physics', 'Chemistry', 'Biology', 'History', 'Geography', 'Literature', 'Fine Art']
...
export default function SecondaryAdminTeachers() {
    ...
    const handleAddNewTeacher = () => {
        ...
        const createdTeacher = {
            id: `TCH-S-${String(teachers.length + 1).padStart(3, '0')}`,
            ...
            email: newTeacher.email || `${newTeacher.name.toLowerCase().replace(/[^a-z]/g, '')}@kss.ug`,
            status: 'active',
            attendance: 100,
            leaveReason: '',
            leaveStart: '',
        ...
        setModal(null)
        resetNewTeacherForm()
        setSuccessMsg(`Secondary Teacher ${createdTeacher.name} successfully added and assigned to ${createdTeacher.class}!`)
    ...
    const saveLeave = () => {
        setTeachers(prev => prev.map(t => t.id === selected.id ? { ...t, status: 'on-leave', leaveReason, leaveStart, leaveEnd, leaveNotes } : t))
        setModal(null)
    ...
    return (
        <DashboardLayout role="schooladmin-secondary">
            <div className="space-y-6 relative">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="page-subtitle">{view === 'staff' ? 'Manage all teaching staff at Kampala Secondary School' : 'Student-submitted teacher absence reports'}</p>
                    </div>
                    {view === 'staff' && (
                        <div className="flex gap-2">
                            <button className="btn-secondary" onClick={() => setModal('import')}><Upload size={15} /> Import</button>
                            <button className="btn-primary bg-indigo-600 hover:bg-indigo-700 text-white" onClick={() => setModal('add')}><Plus size={15} /> Add Teacher</button>
                        </div>
                    )}
                </div>

                <div className="flex items-center border-b border-gray-200 dark:border-slate-700">
                    <button onClick={() => setView('staff')} className={`px-6 py-3 text-sm font-semibold border-b-2 transition-colors ${view === 'staff' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-gray-500 dark:text-slate-400 hover:text-gray-700'}`}>Staff Directory</button>
                    <button onClick={() => setView('reports')} className={`px-6 py-3 text-sm font-semibold border-b-2 transition-colors flex items-center gap-2 ${view === 'reports' ? 'border-red-500 text-red-600' : 'border-transparent text-gray-500 dark:text-slate-400 hover:text-gray-700'}`}>Absence Reports <span className="bg-red-100 dark:bg-red-900/40 text-red-600 dark:text-red-400 py-0.5 px-2 rounded-full text-xs">2</span></button>
                </div>
                            ...
                            <div className="overflow-x-auto"><table className="w-full">
                                ...
                                <tbody>
                                    {filteredTeachers.map(t => (
                                        <tr key={t.id} className="hover:bg-indigo-50/30 dark:hover:bg-slate-700/30 cursor-pointer transition-colors" onClick={(e) => {
                                            ...
                                            <td className="table-cell">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div><p className="text-sm font-semibold text-slate-900 dark:text-white">{t.name}</p><p className="text-xs text-slate-500 dark:text-slate-400">{t.email}</p></div>
                                                </div>
                                            </td>
                                            <td className="table-cell"><Badge variant="info">{t.subject}</Badge></td>
                                            <td className="table-cell text-sm text-slate-600 dark:text-slate-300 font-medium">{t.class}</td>
                                            <td className="table-cell text-sm text-slate-500 dark:text-slate-400">{t.phone}</td>
                                            <td className="table-cell">
                                                <div className="flex items-center gap-2">
                                                    <div className="flex-1 bg-slate-100 dark:bg-slate-700 rounded-full h-1.5 w-16"><div className="bg-emerald-500 h-1.5 rounded-full" style={{ width: `${t.attendance}%` }} /></div>
                                                    <span className="text-xs font-semibold text-slate-700 dark:text-slate-300">{t.attendance}%</span>
                                                </div>
                                            </td>
                            ...
                            <tbody>
                                {absenceReports.map(r => (
                                    <tr key={r.id} className="hover:bg-red-50/30 dark:hover:bg-red-900/10">
                                        <td className="table-cell text-sm font-medium text-slate-900 dark:text-white">{r.date}<p className="text-xs text-slate-500 dark:text-slate-400 font-normal">{r.time}</p></td>
                                        <td className="table-cell font-bold text-slate-900 dark:text-white">{r.teacher}</td>
                                        <td className="table-cell"><Badge variant="gray">{r.subject}</Badge></td>
                                        <td className="table-cell text-sm font-semibold text-indigo-600 dark:text-indigo-400 underline cursor-pointer">{r.reporter}</td>
                                        <td className="table-cell text-sm text-slate-600 dark:text-slate-300 max-w-xs truncate" title={r.details}>{r.details}</td>
                                        <td className="table-cell"><Badge variant={r.status === 'pending' ? 'danger' : 'success'}>{r.status === 'pending' ? 'Pending Review' : 'Reviewed'}</Badge></td>
                                        <td className="table-cell">
                                            {r.status === 'pending' && <button className="btn-secondary text-xs px-2 py-1 flex items-center gap-1 hover:bg-emerald-50 dark:hover:bg-emerald-900/30 hover:text-emerald-700 dark:hover:text-emerald-400 hover:border-emerald-200 dark:border-emerald-700"><CheckCircle size={12} /> Mark Reviewed</button>}
                                        </td>
                                    </tr>
                                ))}
                            </tbody>
            ...
            <Modal isOpen={modal === 'leave'} onClose={() => setModal(null)} title={`Set Leave: ${selected?.name}`}
                footer={<><button className="btn-secondary" onClick={() => setModal(null)}>Cancel</button><button className="btn-primary bg-indigo-600 hover:bg-indigo-700" onClick={saveLeave}><Clock size={14} /> Confirm Leave</button></>}>
                {selected && (
                    <div className="space-y-4">
                        <p className="text-sm text-gray-600 dark:text-slate-300">Mark <strong className="dark:text-white">{selected.name}</strong> as on leave or inactive.</p>
                        <div>
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div><label className="block text-sm font-medium text-gray-700 dark:text-slate-200 mb-1">Start Date</label><input type="date" className="input-field" value={leaveStart} onChange={e => setLeaveStart(e.target.value)} /></div>
                            <div><label className="block text-sm font-medium text-gray-700 dark:text-slate-200 mb-1">Expected Return</label><input type="date" className="input-field" value={leaveEnd} onChange={e => setLeaveEnd(e.target.value)} /></div>
                        </div>
                        <div><label className="block text-sm font-medium text-gray-700 dark:text-slate-200 mb-1">Notes (optional)</label><textarea className="input-field resize-none" rows={2} value={leaveNotes} onChange={e => setLeaveNotes(e.target.value)} placeholder="e.g., Will be attending a 2-week training..." /></div>
                    </div>
                )}
            </Modal>

            <Modal isOpen={modal === 'relocate'} onClose={() => setModal(null)} title={`Relocate/Adjust: ${selected?.name}`}
                footer={<><button className="btn-secondary" onClick={() => setModal(null)}>Cancel</button><button className="btn-primary bg-indigo-600 hover:bg-indigo-700 text-white" onClick={() => setModal(null)}>Save Changes</button></>}>
                {selected && (
                    <div className="space-y-5">
                        <p className="text-sm text-slate-600 dark:text-slate-300">Reassign <strong>{selected.name}</strong> to different subjects or secondary classes.</p>
                        <div>
                            <label className="block text-sm font-bold text-slate-700 dark:text-slate-200 mb-2">Main Subject</label>
                            <select className="input-field w-full" value={relocateSubject} onChange={e => setRelocateSubject(e.target.value)}>
                                {allSecondarySubjects.map(sub => <option key={sub} value={sub}>{sub}</option>)}
                            </select>
                        </div>
                        <div>
                            <label className="block text-sm font-bold text-slate-700 dark:text-slate-200 mb-2">Assigned Classes</label>
                            <div className="flex flex-wrap gap-2">
                                {allSecondaryClasses.map(cls => (
                                    <button key={cls} onClick={() => toggleClassSelection(cls)}
                                        className={`px-3 py-1.5 rounded-lg text-sm font-bold transition-all shadow-sm ${relocateClasses.includes(cls) ? 'bg-indigo-100 dark:bg-indigo-900/40 text-indigo-800 dark:text-indigo-300 ring-2 ring-indigo-500' : 'bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 text-slate-600 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-600'}`}
                                    >{cls}</button>
                                ))}
                            </div>
                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-2 font-medium">Selected: {relocateClasses.join(', ') || 'None'}</p>
                        </div>
                    </div>
                )}
            </Modal>
            ...
            <Modal
                ...
                title="Add New Secondary Teacher"
                size="lg"
                footer={
                    <>
                        <button className="btn-secondary" onClick={() => { setModal(null); resetNewTeacherForm() }}>Cancel</button>
                        <button className="btn-primary bg-indigo-600 hover:bg-indigo-700 text-white" onClick={handleAddNewTeacher}>
            ...
            >
                <div className="space-y-4">
                    ...
                    <div className="p-3.5 bg-slate-50 dark:bg-slate-800/70 rounded-xl border border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row items-center gap-4">
                        <div className="relative group">
                            {newTeacher.photo ? (
                                ...
                                <div className="w-16 h-16 rounded-2xl bg-indigo-100 dark:bg-slate-700 border-2 border-dashed border-indigo-300 dark:border-slate-600 flex flex-col items-center justify-center text-indigo-600 dark:text-indigo-400">
                                    <Camera size={20} />
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input
                                ...
                                placeholder="e.g., Dr. Francis Kigozi"
                        ...
                        <div>
                            ...
                            <input
                                type="email"
                                className="input-field"
                                placeholder="f.kigozi@kss.ug"
                                value={newTeacher.email}
                                onChange={e => setNewTeacher({ ...newTeacher, email: e.target.value })}
                            />
                        </div>
                        <div>
                            <label className="block text-xs font-bold text-slate-700 dark:text-slate-200 mb-1">Phone Number *</label>
                            <input
                                type="tel"
                                className="input-field"
                        ...
                        <div>
                            <label className="block text-xs font-bold text-slate-700 dark:text-slate-200 mb-1">Main Subject</label>
                            <select
                    ...
                    <div className="p-3.5 bg-slate-50 dark:bg-slate-800/60 rounded-xl border border-slate-200 dark:border-slate-700 space-y-2">
                        <div className="flex items-center justify-between">
                            <label className="block text-xs font-bold text-slate-700 dark:text-slate-200">
                                Assigned Secondary Classes <span className="text-slate-400 font-normal">(Click multiple classes to assign)</span>
                            </label>
            ...
            <Modal isOpen={modal === 'view'} onClose={() => setModal(null)} title="Teacher Details"
                footer={<button className="btn-primary bg-indigo-600 hover:bg-indigo-700 text-white" onClick={() => setModal(null)}>Close</button>}>
                {selected && (
                    <div className="space-y-4">
                        <div className="flex items-center gap-4 border-b border-slate-100 dark:border-slate-700 pb-4">
                            {selected.photo ? (
                                ...
                                <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center text-white text-2xl font-bold flex-shrink-0">{selected.name.split(' ').pop()[0]}</div>
                            )}
                            <div><p className="text-lg font-black text-slate-900 dark:text-white">{selected.name}</p><p className="text-sm font-medium text-slate-500 dark:text-slate-400">{selected.email}</p></div>
                        </div>
                        <div className="grid grid-cols-2 gap-4 text-sm">
                            {[['Subject', selected.subject], ['Classes', selected.class], ['Phone', selected.phone], ['Attendance Rate', `${selected.attendance}%`], ['Status', selected.status === 'on-leave' ? 'On Leave' : selected.status], ['Staff ID', selected.id]].map(([k, v]) => (
                                <div key={k} className="bg-slate-50 dark:bg-slate-700 border border-slate-100 dark:border-slate-600 p-3 rounded-xl"><p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide">{k}</p><p className="font-bold text-slate-900 dark:text-white mt-1">{v}</p></div>
                            ))}
                        </div>
