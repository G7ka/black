            ...
            <div className="space-y-6">
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    ...
                    <div className="flex items-center gap-2">
                        <label className="text-sm font-medium text-gray-600 dark:text-slate-300">Date:</label>
                ...
                <div>
                    ...
                    <div className="card p-0">
                        <div className="overflow-x-auto"><table className="w-full">
                            ...
                            <tbody>
                                {classData.map(c => (
                                    <tr key={c.name} className="hover:bg-blue-50/30 dark:hover:bg-slate-800/50 transition-colors">
                                        <td className="table-cell font-semibold text-gray-900 dark:text-white">{c.name}</td>
                                        <td className="table-cell text-sm">{c.total}</td>
                                        <td className="table-cell text-sm text-emerald-600 dark:text-emerald-400 font-medium">{c.present}</td>
                                        <td className="table-cell text-sm text-red-500 dark:text-red-400 font-medium">{c.absent}</td>
                                        <td className="table-cell">
                                            <div className="flex items-center gap-2">
                                                <div className="flex-1 h-2 bg-gray-100 dark:bg-slate-700 rounded-full max-w-[100px]">
                                                    <div className="h-2 bg-emerald-500 rounded-full" style={{ width: c.rate }} />
                                                </div>
                                                <span className="text-xs font-semibold text-gray-700 dark:text-slate-300">{c.rate}</span>
                                            </div>
                                        </td>
                                    </tr>
                                ))}
